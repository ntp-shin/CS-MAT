...
class TwoStageLoss(Loss):
    ...
    def run_G(self, img_in, mask_in, channel_in, z, c, sync):
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            img, img_stg1 = self.G_synthesis(img_in, mask_in, channel_in, ws, return_stg1=True)
    ...
    def accumulate_gradients(self, phase, real_img, mask, channel, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, gen_img_stg1 = self.run_G(real_img, mask, channel, gen_z, gen_c, sync=(sync and not do_Gpl)) # May get synced by Gpl.
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws, gen_img_stg1 = self.run_G(real_img, mask, channel, gen_z, gen_c, sync=False)
