...
snapshot_path = '/media/nnthao/MAT/saved_model/cmat_feature_snapshots/'
def save_feature_snapshot(tensor, stage_name, show_stat=False, is_m=True):
    print('Stage by order:', stage_name)
    norm_tensor = (tensor[0].mean(dim=0, keepdim=True).permute(1, 2, 0) * torch.Tensor([[[1, 1, 1]]]).to('cpu' if tensor.get_device() == -1 else tensor.get_device())).detach().cpu()

    if norm_tensor.min() < 0 or norm_tensor.max() > 1:
        if is_m:
            norm_tensor /=  norm_tensor.max()
        else:
            norm_tensor = (norm_tensor - norm_tensor.min()) / (norm_tensor.max() - norm_tensor.min())
        plt.imsave(f'{snapshot_path}{stage_name}.png', norm_tensor.detach().numpy(), vmin=0, vmax=1)
    else:
        plt.imsave(f'{snapshot_path}{stage_name}.png', norm_tensor.detach().numpy(), vmin=0, vmax=1)

    if show_stat:
        print(f'\n{stage_name}', tensor.shape, tensor.min().item(), tensor.mean().item(), tensor.max().item())
        print('norm_tensor', norm_tensor.shape, norm_tensor.min().item(), norm_tensor.mean().item(), norm_tensor.max().item())
    print()
...
class Conv2dLayerPartial(nn.Module):
    def __init__(self,
        ...
        self.stride = down
        self.transpose_stride = up
        self.padding = kernel_size // 2 if kernel_size % 2 == 1 else 0
        
        assert up == 1 and down == 1 or up == 2 and down == 1 or up == 1 and down == 2
        # self.mconv = Conv2dLayer(in_channels, out_channels, kernel_size, bias, activation, up, down, resample_filter,
        #                         conv_clamp, trainable)

    def forward(self, x, mask=None, post_tran_conv=False):
        if mask is not None:
            # with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                update_mask = update_mask / (1e-6 + torch.amax(update_mask, dim=(2, 3), keepdim=True))
            # update_mask = self.mconv(mask)
...
class WindowAttention(nn.Module):
    ...
    def to_features(self, tokens, B_, B):
        """
        Args:
            tokens: (B_, N, C)
        Returns:
            features: (B, head, L, C_)
        """
        L = int(B_/B * self.window_size[0] * self.window_size[1])
        H = W = int(np.sqrt(L))
        features = tokens.reshape(B, H // self.window_size[0], W // self.window_size[1], self.window_size[0], self.window_size[1], self.num_heads, -1)
        features = features.permute(0, 5, 1, 3, 2, 4, 6).reshape(B, self.num_heads, H, W, -1).reshape(B, self.num_heads, L, -1)
        return features

    def to_tokens(self, features):
        """
        Args:
            features: (B, head, L, C_)
        Returns:
            tokens: (B_, head, N, C_)
        """
        B, _, L, C_ = features.shape
        H = W = int(np.sqrt(L))
        features = features.reshape(B, self.num_heads, H, W, -1).reshape(B, self.num_heads, H // self.window_size[0], self.window_size[0], W // self.window_size[1], self.window_size[1], -1)
        tokens = features.permute(0, 2, 4, 1, 3, 5, 6).reshape(-1, self.num_heads, self.window_size[0] * self.window_size[1], C_)
        return tokens

    def forward(self, x, mask_windows, mask=None, B=1):
        """
        Args:
        ...
        H = W = int(np.sqrt(L))
        global attn_count
        if attn_count == 1 or attn_count == 13:
            shift_size = 4
        elif attn_count == 3 or attn_count == 10:
            shift_size = 8
        else:
            shift_size = 0
        ...
        # save_feature_snapshot(torch.roll(window_reverse(q.permute(0, 2, 1, 3).reshape(B_, N, -1), self.window_size[0], H, W).permute(0, 3, 1, 2), shifts=(shift_size, shift_size), dims=(2, 3)), f'q{attn_count}', is_m=False)
        # save_feature_snapshot(torch.roll(window_reverse(k.permute(0, 3, 1, 2).reshape(B_, N, -1), self.window_size[0], H, W).permute(0, 3, 1, 2), shifts=(shift_size, shift_size), dims=(2, 3)), f'k{attn_count}', is_m=False)
        # save_feature_snapshot(torch.roll(window_reverse(v.permute(0, 2, 1, 3).reshape(B_, N, -1), self.window_size[0], H, W).permute(0, 3, 1, 2), shifts=(shift_size, shift_size), dims=(2, 3)), f'v{attn_count}', is_m=False)

        with torch.no_grad():
            # q_m = self.to_features(mask_windows @ torch.abs(self.q.weight.transpose(1, 0)), B_, B)
            # k_m = self.to_features(mask_windows @ torch.abs(self.k.weight.transpose(1, 0)), B_, B)
            # v_m = self.to_features(mask_windows @ torch.abs(self.v.weight.transpose(1, 0)), B_, B)
            ...
            # q_m = self.to_tokens(q_m)
            # k_m = self.to_tokens(k_m).permute(0, 1, 3, 2)
            # v_m = self.to_tokens(v_m)

            q_m = (mask_windows @ torch.abs(self.q.weight.transpose(1, 0))).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            k_m = (mask_windows @ torch.abs(self.k.weight.transpose(1, 0))).view(B_, -1, self.num_heads, C // self.num_heads).permute(0, 2, 3, 1)
            v_m = (mask_windows @ torch.abs(self.v.weight.transpose(1, 0))).view(B_, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            ...
            # save_feature_snapshot(torch.roll(window_reverse(q_m.permute(0, 2, 1, 3).reshape(B_, N, -1), self.window_size[0], H, W).permute(0, 3, 1, 2), shifts=(shift_size, shift_size), dims=(2, 3)), f'q_m{attn_count}')
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size, mask=None):
        ...
        if self.input_resolution == x_size:
            attn_windows, mask_windows = self.attn(x_windows, mask_windows, mask=self.attn_mask, B=B)  # nW*B, window_size*window_size, C
        else:
...
import matplotlib.pyplot as plt, PIL.Image
@persistence.persistent_class
